...
### Important Change
Remember to implement selective weight decay for the norm layers and bias terms in the optimizer, as the final output of each encoder block is an affine normalization layer. Without this, the final activation of the encoder is pushed to be small as the weights of the norm is a scaling value to it. This will cause the internal layers of the encoder also push their activations to be small and lowering their weights and eventally collapsing. This is a common practice in training transformers to improve generalization and stability.
...
|---|---|---|---|
| Small | 4M | 80,000 | ~66% (plateau) |
| Medium (ViT-Base) | 114M | 80,000 | ~59% (plateau) |

A fully supervised CNN reaches ~85%. The larger model achieving worse probe accuracy than the smaller one is a sign of overfitting: the 114M encoder memorises the 100k unlabelled images and fails to generalise to the 5k labelled images.
